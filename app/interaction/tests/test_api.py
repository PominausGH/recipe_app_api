    ...
    def test_follow_private_user_creates_request(self):
        ...
        self.assertTrue(
            FollowRequest.objects.filter(
                requester=self.user1, target=self.private_user
            ).exists()
        )
...
class FollowRequestAPITests(TestCase):
    ...
    def test_accept_follow_request(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertTrue(
            Follow.objects.filter(follower=self.requester, following=self.user).exists()
        )
...
class BlockMuteAPITests(TestCase):
    ...
    def test_block_user(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            Block.objects.filter(user=self.user1, blocked_user=self.user2).exists()
        )
    ...
    def test_mute_user(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            Mute.objects.filter(user=self.user1, muted_user=self.user2).exists()
        )
